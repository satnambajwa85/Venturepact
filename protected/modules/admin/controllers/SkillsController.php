...
class SkillsController extends Controller
{
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Skills']))
		{
			...
			if(isset($_FILES["Skills"]["tmp_name"]['image']))
			{
				$userfile_extn = explode("/", strtolower($_FILES['Skills']['type']['image']));
				$newNamw	=	time().'.'.$userfile_extn[1];
				$fileTmpLoc = $_FILES["Skills"]["tmp_name"]['image'];
				// Path and file name
				$pathAndName = "images/skills/".$newNamw;
				// Run the move_uploaded_file() function here
				$moveResult = move_uploaded_file($fileTmpLoc, $pathAndName);
				$model->logo	=	"skills/".$newNamw;
			}
			if($model->save())
