...
class Skills extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('skill_category_id', 'required'),
			array('status, skill_category_id', 'numerical', 'integerOnly'=>true),
			array('title, image', 'length', 'max'=>45),
			array('description', 'length', 'max'=>255),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, title, description, image, status, skill_category_id', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'developerHasSkills' => array(self::HAS_MANY, 'DeveloperHasSkills', 'skills_id'),
			'frameworks' => array(self::HAS_MANY, 'Frameworks', 'skills_id'),
			'projectsHasSkills' => array(self::HAS_MANY, 'ProjectsHasSkills', 'skills_id'),
